...
- **Earlier 2026-06-20 batch:** checklist/label add bugfix, deeper 3D buttons, open-ended task priority, daily to-do planner. Migrations `20260620100000_card_priority.sql` (applied live) + `20260620120000_todos.sql` (**still to apply**).
- **Next step:** Run **Phase 8 (Collaboration)** from [prompt.md](./prompt.md) / `prompts.html` — invite members, roles (owner/editor/viewer), and Supabase Realtime live updates. Apply the pending migrations first (`20260620120000_todos.sql`, `20260620140000_notes.sql`), then run the live verifications (below).
...
└── src/
    ...
    │       ├── NotesPanel.tsx      ← two-pane list + editor (responsive list↔editor swap), create
    │       ├── NoteEditor.tsx      ← title rename + markdown textarea + live preview + debounced autosave + delete
...
- **2026-06-20 migrations.** `20260620100000_card_priority.sql` applied live ✓. **`20260620120000_todos.sql` — apply before using the To-Do tab** (adds `todo_lists` + `todo_items` + `owns_todo_list()` + own-row RLS; idempotent). **`20260620140000_notes.sql` — apply before using the Notes tab** (adds `notes` + `touch_notes_updated_at()` trigger + member-gated RLS; idempotent).
...
- **Bundle size:** the single JS chunk is now ~917 kB (266 kB gzip) — still a build warning, not an error (the markdown renderer added no dependency). Code-splitting is a Phase 10 polish item.
...
Open `prompt.md` (or `prompts.html`) → **Phase 8 — Collaboration**: invite members to a project, assign roles (owner/editor/viewer), and add Supabase **Realtime** so board/card/notes changes stream live between members. The role limits (e.g. viewers read-only) tighten the currently membership-only RLS on `columns`/`cards`/`checklist_items`/`labels`/`card_labels`/`notes` — plan the policy changes against `plan.md` §6. Reuse the existing optimistic-cache + Zod + glass patterns; keep it responsive.

**Still to apply (Supabase SQL editor / `db push`): `20260620120000_todos.sql`** (To-Do tab) and **`20260620140000_notes.sql`** (Notes tab). Then run the **Phase 4–7 + 2026-06-20-batch live verifications** in the open-items list above (board CRUD + drag-and-drop; checklists/due-dates/labels/**priority**; toolbar filtering/search; calendar month/week/scope/drag-reschedule; the **To-Do** tab; the **Notes** tab — autosave/preview/RLS; RLS isolation against a second account). Earlier one-time setup (real `.env` keys, profiles + projects migrations, Email/Google providers) is done. **Also push `main` to origin** — it's ahead by Phase 6, the 2026-06-20 batch, and Phase 7.
