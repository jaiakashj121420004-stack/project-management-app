...
- **2026-06-18** — **UI redesign → "Cosmic Bento"** (branch `redesign/cosmic-bento`, pending preview review): elevated the Aurora design system — deep-space nebula (dark) + dawn-nebula (light) backgrounds with starfield, drifting aurora ribbons, film grain, and 3-plane pointer parallax (`AuroraBackground`); layered/dimensional glass with a cursor-tracked spotlight + hover lift (`GlassCard`); chunky tactile 3D buttons (solid accent base-edge that depresses on press); and a bento-grid Projects dashboard with a featured hero tile. Light + dark are both first-class and equally rich. All shared class/variable contracts preserved, so every screen inherits the upgrade — no features changed. tsc/build/lint clean.
