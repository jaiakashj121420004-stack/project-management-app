...
- **Next step:** Run **Phase 7 (Notes/docs per project)** from [prompt.md](./prompt.md) / `prompts.html` — a `notes` table (RLS by membership) + a per-project Notes tab with markdown editor + debounced autosave. Before that, apply the still-pending Phase 4 + Phase 5 migrations and run the live verifications (below).
...
Before that, apply the still-pending migrations in the Supabase dashboard in order — the **Phase 4** (`20260618140000_kanban.sql`) and **Phase 5** (`20260618160000_card_details.sql`) files — then run the **Phase 4 + Phase 5 + Phase 6 live verification** in the open-items list above (board CRUD + drag-and-drop; checklists/due-dates/labels; toolbar filtering/search; the calendar month/week/scope/drag-reschedule; RLS isolation against a second account). The earlier one-time setup (real `.env` keys, profiles + projects migrations, Email/Google providers) is already done. **Phase 6 added no migration.**
