/**
 * Fractional ("between") positional ordering — shared by every drag-reorderable
 * list in the app (Kanban columns/cards, to-do lists/items, …).
 *
 * Each row carries a numeric `position`; lists render in ascending order. To
 * move an item we give it a position *between* its two new neighbours — the
 * midpoint of their positions — so a reorder writes exactly one row and can
 * never collide with an existing position. This is a fractional strategy, not
 * a full reindex: the rest of the list is left untouched, which keeps moves
 * cheap and optimistic updates trivial.
 *
 ...
 *
 * Every helper here is generic over `{ position; created_at?; id? }`, so it has
 * no dependency on any particular table's row shape — the board (Card/Column)
 * and the to-do planner (TodoList/TodoItem) both import from here directly
 * rather than duplicating this module.
 */
...
/**
 * Ascending-by-position comparator shared by every ordered list (columns,
 * cards, checklist items, to-do lists/items, …).
 *
...
/**
 * The position a moved item should take to sit at its new index: the midpoint
 * of the positions of the items that now straddle it (the moved item itself is
 * skipped). `order` is the final id order of the destination list — typically
 * the result of `arrayMove(ids, oldIndex, newIndex)` from `@dnd-kit/sortable`.
 */
