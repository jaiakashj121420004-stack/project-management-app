...
| `time_entries` | Per-card start/stop time tracking (v1: card-only, no billable rates/invoicing/reports) | `id`, `card_id`, `user_id`, `started_at`, `ended_at` (null = running) |
...
**Feature → data mapping:** Multiple projects → `projects`. Kanban → `columns` + `cards` (`position` for ordering). To-do lists → `checklist_items`. Time tracking → `time_entries` (card-only, v1). Due dates & reminders → `cards.due_date` (+ reminders in Phase 9). Calendar → query cards with a `due_date`. Notes → `notes`. Canvas → `canvas_notes`. Collaboration → `project_members` + Realtime.
