...
| `project_share_links` | Public read-only board share link (2026-08-16) | `id`, `project_id`, `token` (unguessable, unique), `created_by`, `revoked_at?`, `created_at` |
...
- **Public read-only project share links (2026-08-16).** A project owner can turn on a `/share/:token` link that renders a stripped-down, read-only board (columns/cards/labels/due dates — no comments, no edit affordances, no member list) with no login. The token is DB-generated, unguessable, and revocable (`project_share_links`, owner-only RLS via `is_project_owner()`); the public read itself goes through a dedicated Edge Function (`project-share`, `--no-verify-jwt`, service role, rate-limited by IP) that always returns an identical 404 for a missing/malformed/unknown/revoked token — the same "never distinguish, never leak which tokens exist" discipline as the calendar feed. See `supabase/migrations/20260816150000_project_share_links.sql` for the full design rationale (why this is plain-RLS-on-its-own-table rather than a protected-column + Edge-Function pattern like the calendar feed token).
